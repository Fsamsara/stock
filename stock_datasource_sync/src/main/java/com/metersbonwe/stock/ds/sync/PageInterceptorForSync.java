 ...
 * @author zhangfeng
 */
@Intercepts({ @Signature(method = "prepare", type = StatementHandler.class, args = { Connection.class }) }) public class PageInterceptorForSync
        implements Interceptor {
    private final static StockLog LOGGER = StockLogFactory.getLogger(PageInterceptorForSync.class);
    ...
    private String getPageSql(String sql, Page<?> page) {
        ...
        sqlBuffer.insert(0, sqlPre).append(" ) u where rownum <= ").append(page.getPageNo() * page.getPageSize()).append(" ) where r > ")
